...
| **OI-flow fade** | `oi-flow-fade` | heavy 5-day call-OI build → underperformance. **The shipped rule measures −0.65%, 34 `lane-exit-day`, p=0.093 — still NEGATIVE, no longer BH-significant** (NW p=0.290), which fired outcome 2 of the pre-registered re-check. *(Stale figures retired 2026-08-29: the "+1.21% median, n=906, most robust edge, HIGH" reading was a 2026-07-24 raw-rank baseline, superseded three times — full-window fix 08-22, live-rule grading 08-29, re-check 09-05.)* | swing10 | **ADVISORY — emits candidates at `watch`; never sizes** |
