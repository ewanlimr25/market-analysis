...
def _borrow_for_date(load_borrow: Callable, d) -> pd.DataFrame:
    result = load_borrow(d)
    data = result["data"] if isinstance(result, dict) else result
    if data is None or data.empty:
        return pd.DataFrame(columns=["symbol", "borrow_fee"])
    return data[_BORROW_KEEP].rename(columns={"fee_rate": "borrow_fee"})
...
def join_short_side(spine: pd.DataFrame, load_short_interest: Callable = SI.load_short_interest,
                     load_borrow: Callable = B.load_borrow) -> pd.DataFrame:
    ...
    known that day) and `borrow_fee` (that day's IBKR snapshot, nullable when the mart has no snapshot
    for the day)."""
    ...
    for d, group in spine.groupby("date", sort=False):
        ...
        borrow = _borrow_for_date(load_borrow, d).drop_duplicates("symbol")
