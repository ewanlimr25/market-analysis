...
2. **Liquidity floor (C12), fail-closed.** price ≥ $5 AND 20-day $-ADV ≥ $50M; a name you cannot verify is
   dropped, not sized (`RESEARCH/30` flagged illiquidity contamination on S2/S4).
...
the validated-excess size is a violation. The regression gate (`retro_harness.py --all`) must show no
lane/tier negative-excess after any change you propose.
